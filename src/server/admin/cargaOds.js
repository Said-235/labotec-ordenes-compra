...
import { eliminarProductosPorCarga, productoEnOrdenes } from './eliminarProductos.js'

export { eliminarProductosPorCarga }
import { nombreCategoria } from '../../lib/categorias.js'
import { listarCategorias } from './categorias.js'
import { mapCategorias } from '../../lib/categorias.js'
import {
  ...
  requiereGrupoPrueba,
  resolveGrupoPrueba,
} from '../../lib/grupoPrueba.js'
import { sanitizeRowErrorMessage } from '../../lib/errors.js'
...
/**
 * Persiste filas ya parseadas en el cliente (el ODS no viaja al servidor).
 */
export async function procesarCargaODSFromRows({
  categoria,
  validRows = [],
  errors: initialErrors = [],
  totalFilas = 0,
  nombreArchivo = 'carga.ods',
}) {
  ...
  const categoriaKeys = categoriasRows.filter((r) => r.activo).map((r) => r.clave)
  ...
  const filasUnicas = deduplicarFilas(
    (validRows ?? []).map((row) => ({ ...row, categoria })),
  )
  ...
  const { error: logError } = await admin.from('log_cargas').insert({
    admin_id: user.id,
    categoria,
    nombre_archivo: sanitizeText(nombreArchivo, 255),
